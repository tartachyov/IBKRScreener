...
var function_rate_limit_1 = __importDefault(require("function-rate-limit"));
...
Outgoing.prototype._send = function_rate_limit_1.default(constants_1.default.MAX_REQ_PER_SECOND, 1000, function () {
    var args = Array.prototype.slice.call(arguments);
    this._controller.run('send', lodash_1.default.flatten(args));
});
Outgoing.prototype.calculateImpliedVolatility = function (reqId, contract, optionPrice, underPrice) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.REQ_CALC_IMPLIED_VOLAT) {
        return this._controller.emitError('It does not support calculate implied volatility requests.');
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.TRADING_CLASS) {
        if (!lodash_1.default.isEmpty(contract.tradingClass)) {
            return this._controller.emitError('It does not support tradingClass parameter in calculateImpliedVolatility.');
        }
    }
    var version = 2;
    var args = [constants_1.default.OUTGOING.REQ_CALC_IMPLIED_VOLAT, version, reqId];
...
Outgoing.prototype.calculateOptionPrice = function (reqId, contract, volatility, underPrice) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.REQ_CALC_OPTION_PRICE) {
    ...
    var version = 2;
    var args = [constants_1.default.OUTGOING.REQ_CALC_OPTION_PRICE, version, reqId];
    // send contract fields
    args.push(contract.conId);
    args.push(contract.symbol);
    args.push(contract.secType);
...
Outgoing.prototype.cancelAccountSummary = function (reqId) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.ACCT_SUMMARY) {
        return this._controller.emitError('It does not support account summary cancellation.');
    }
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_ACCOUNT_SUMMARY, version, reqId);
};
Outgoing.prototype.cancelAccountUpdatesMulti = function (reqId) {
    var version = 2;
    this._send(constants_1.default.OUTGOING.CANCEL_ACCOUNT_UPDATES_MULTI, version, reqId);
};
Outgoing.prototype.cancelPositionsMulti = function (reqId) {
    var version = 2;
    this._send(constants_1.default.OUTGOING.CANCEL_POSITIONS_MULTI, version, reqId);
};
Outgoing.prototype.cancelCalculateImpliedVolatility = function (reqId) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.CANCEL_CALC_IMPLIED_VOLAT) {
        return this._controller.emitError('It does not support calculate implied volatility cancellation.');
    }
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_CALC_IMPLIED_VOLAT, version, reqId);
};
Outgoing.prototype.cancelCalculateOptionPrice = function (reqId) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.CANCEL_CALC_OPTION_PRICE) {
        return this._controller.emitError('It does not support calculate option price cancellation.');
    }
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_CALC_OPTION_PRICE, version, reqId);
};
Outgoing.prototype.cancelFundamentalData = function (reqId) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.FUNDAMENTAL_DATA) {
        return this._controller.emitError('It does not support fundamental data requests.');
    }
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_FUNDAMENTAL_DATA, version, reqId);
};
Outgoing.prototype.cancelHistoricalData = function (tickerId) {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_HISTORICAL_DATA, version, tickerId);
};
Outgoing.prototype.cancelMktData = function (tickerId) {
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_MKT_DATA, version, tickerId);
};
Outgoing.prototype.cancelMktDepth = function (tickerId) {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_MKT_DEPTH, version, tickerId);
};
Outgoing.prototype.cancelNewsBulletins = function () {
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_NEWS_BULLETINS, version);
};
Outgoing.prototype.cancelOrder = function (id) {
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_ORDER, version, id);
};
Outgoing.prototype.cancelPositions = function () {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_POSITIONS, version);
};
Outgoing.prototype.cancelRealTimeBars = function (tickerId) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.REAL_TIME_BARS) {
        return this._controller.emitError('It does not support realtime bar data query cancellation.');
    }
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_REAL_TIME_BARS, version, tickerId);
};
Outgoing.prototype.cancelScannerSubscription = function (tickerId) {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.CANCEL_SCANNER_SUBSCRIPTION, version, tickerId);
};
Outgoing.prototype.exerciseOptions = function (tickerId, contract, exerciseAction, exerciseQuantity, account, override) {
    ...
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.TRADING_CLASS) {
        if (!lodash_1.default.isEmpty(contract.tradingClass) || contract.conId > 0) {
            return this._controller.emitError('It does not support conId and tradingClass parameters in exerciseOptions.');
        }
    }
    var args = [constants_1.default.OUTGOING.EXERCISE_OPTIONS, version, tickerId];
    // send contract fields
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.TRADING_CLASS) {
        args.push(contract.conId);
    }
...
Outgoing.prototype.placeOrder = function (id, contract, order) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.SCALE_ORDERS) {
    ...
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.SSHORT_COMBO_LEGS) {
        if (lodash_1.default.isArray(contract.comboLegs)) {
            contract.comboLegs.forEach(function (comboLeg) {
                if (comboLeg.shortSaleSlot !== 0 || !lodash_1.default.isEmpty(comboLeg.designatedLocation)) {
                    return this._controller.emitError('It does not support SSHORT flag for combo legs.');
                }
            }.bind(this));
        }
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.WHAT_IF_ORDERS) {
        if (order.whatIf) {
            return this._controller.emitError('It does not support what-if orders.');
        }
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.UNDER_COMP) {
        if (contract.underComp) {
            return this._controller.emitError('It does not support delta-neutral orders.');
        }
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.SCALE_ORDERS2) {
    ...
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.ALGO_ORDERS) {
        if (!lodash_1.default.isEmpty(order.algoStrategy)) {
            return this._controller.emitError('It does not support algo orders.');
        }
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.NOT_HELD) {
        if (order.notHeld) {
            return this._controller.emitError('It does not support notHeld parameter.');
        }
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.SEC_ID_TYPE) {
        if (!lodash_1.default.isEmpty(contract.secIdType) || !lodash_1.default.isEmpty(contract.secId)) {
            return this._controller.emitError('It does not support secIdType and secId parameters.');
        }
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.PLACE_ORDER_CONID) {
        if (contract.conId > 0) {
            return this._controller.emitError('It does not support conId parameter.');
        }
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.SSHORTX) {
    ...
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.HEDGE_ORDERS) {
        if (!lodash_1.default.isEmpty(order.hedgeType)) {
            return this._controller.emitError('It does not support hedge orders.');
        }
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.OPT_OUT_SMART_ROUTING) {
        if (order.optOutSmartRouting) {
            return this._controller.emitError('It does not support optOutSmartRouting parameter.');
        }
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.DELTA_NEUTRAL_CONID) {
        if (order.deltaNeutralConId > 0 ||
            !lodash_1.default.isEmpty(order.deltaNeutralSettlingFirm) ||
            !lodash_1.default.isEmpty(order.deltaNeutralClearingAccount) ||
            !lodash_1.default.isEmpty(order.deltaNeutralClearingIntent)) {
    ...
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.DELTA_NEUTRAL_OPEN_CLOSE) {
        if (!lodash_1.default.isEmpty(order.deltaNeutralOpenClose) ||
            order.deltaNeutralShortSale ||
            order.deltaNeutralShortSaleSlot > 0 ||
            !lodash_1.default.isEmpty(order.deltaNeutralDesignatedLocation)) {
    ...
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.SCALE_ORDERS3) {
    ...
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.ORDER_COMBO_LEGS_PRICE &&
        lodash_1.default.isString(contract.secType) &&
        constants_1.default.BAG_SEC_TYPE.toUpperCase() === contract.secType.toUpperCase()) {
        if (lodash_1.default.isArray(order.orderComboLegs)) {
    ...
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.TRAILING_PERCENT) {
    ...
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.SCALE_TABLE) {
        if (!lodash_1.default.isEmpty(order.scaleTable) ||
            !lodash_1.default.isEmpty(order.activeStartTime) ||
            !lodash_1.default.isEmpty(order.activeStopTime)) {
            return this._controller.emitError('It does not support scaleTable, activeStartTime and activeStopTime parameters.');
        }
    }
    var version = (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.NOT_HELD ? 27 : 41);
    // send place order msg
    var args = [constants_1.default.OUTGOING.PLACE_ORDER, version, id];
    // send contract fields
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.PLACE_ORDER_CONID) {
        args.push(contract.conId);
    }
    ...
    if (this._controller._serverVersion >= 8 &&
        ...
        if (!lodash_1.default.isArray(contract.comboLegs)) {
            ...
            contract.comboLegs.forEach(function (comboLeg) {
                ...
                if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.SSHORTX_OLD) {
    ...
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.SMART_COMBO_ROUTING_PARAMS &&
        lodash_1.default.isString(contract.secType) &&
        constants_1.default.BAG_SEC_TYPE.toUpperCase() === contract.secType.toUpperCase()) {
        smartComboRoutingParamsCount = !lodash_1.default.isArray(order.smartComboRoutingParams) ? 0 : order.smartComboRoutingParams.length;
    ...
    if (this._controller._serverVersion >= 26) { // Volatility orders
        ...
        if (this._controller._serverVersion < 28) {
            ...
            if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.DELTA_NEUTRAL_CONID &&
                !lodash_1.default.isEmpty(order.deltaNeutralOrderType)) {
    ...
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.PTA_ORDERS) {
    ...
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.UNDER_COMP) {
        if (lodash_1.default.isPlainObject(contract.underComp) && !lodash_1.default.isEmpty(contract.underComp)) {
            args.push(true);
    ...
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.ALGO_ORDERS) {
        ...
        if (!lodash_1.default.isEmpty(order.algoStrategy)) {
            algoParamsCount = (!lodash_1.default.isArray(order.algoParams) ? 0 : order.algoParams.length);
...
Outgoing.prototype.replaceFA = function (faDataType, xml) {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.REPLACE_FA, version, faDataType, xml);
};
Outgoing.prototype.reqAccountSummary = function (reqId, group, tags) {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_ACCOUNT_SUMMARY, version, reqId, group, tags);
};
Outgoing.prototype.reqAccountUpdates = function (subscribe, acctCode) {
    var version = 2;
    if (this._controller._serverVersion >= 9) {
        this._send(constants_1.default.OUTGOING.REQ_ACCOUNT_DATA, version, subscribe, acctCode);
    }
    else {
        this._send(constants_1.default.OUTGOING.REQ_ACCOUNT_DATA, version, subscribe);
    }
};
Outgoing.prototype.reqAccountUpdatesMulti = function (reqId, acctCode, modelCode, ledgerAndNLV) {
    var version = 2;
    this._send(constants_1.default.OUTGOING.REQ_ACCOUNT_UPDATES_MULTI, version, reqId, acctCode, modelCode, ledgerAndNLV);
};
Outgoing.prototype.reqAllOpenOrders = function () {
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_ALL_OPEN_ORDERS, version);
};
Outgoing.prototype.reqAutoOpenOrders = function (bAutoBind) {
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_AUTO_OPEN_ORDERS, version, bAutoBind);
};
Outgoing.prototype.reqHeadTimestamp = function (reqId, contract, whatToShow, useRTH, formatDate) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.REQ_HEAD_TIMESTAMP) {
    ...
    var args = [constants_1.default.OUTGOING.REQ_HEAD_TIMESTAMP,
        reqId,
        contract.conId,
        contract.symbol,
        contract.secType,
...
Outgoing.prototype.reqContractDetails = function (reqId, contract) {
    ...
    var args = [constants_1.default.OUTGOING.REQ_CONTRACT_DATA, version];
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.CONTRACT_DATA_CHAIN) {
        args.push(reqId);
    }
    // send contract fields
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.CONTRACT_CONID) {
        args.push(contract.conId);
    }
    args.push(contract.symbol);
    args.push(contract.secType);
...
Outgoing.prototype.reqCurrentTime = function () {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_CURRENT_TIME, version);
};
Outgoing.prototype.reqExecutions = function (reqId, filter) {
    // NOTE: Time format must be 'yyyymmdd-hh:mm:ss' E.g. '20030702-14:55'
    var version = 3;
    // send req open orders msg
    var args = [constants_1.default.OUTGOING.REQ_EXECUTIONS, version];
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.EXECUTION_DATA_CHAIN) {
        args.push(reqId);
    }
...
Outgoing.prototype.reqFundamentalData = function (reqId, contract, reportType) {
    ...
    var args = [constants_1.default.OUTGOING.REQ_FUNDAMENTAL_DATA, version, reqId];
    // send contract fields
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.TRADING_CLASS) {
        args.push(contract.conId);
    }
...
Outgoing.prototype.reqGlobalCancel = function () {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.REQ_GLOBAL_CANCEL) {
        return this._controller.emitError('It does not support globalCancel requests.');
    }
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_GLOBAL_CANCEL, version);
};
Outgoing.prototype.reqHistoricalData = function (tickerId, contract, endDateTime, durationStr, barSizeSetting, whatToShow, useRTH, formatDate, keepUpToDate) {
    ...
    var args = [constants_1.default.OUTGOING.REQ_HISTORICAL_DATA, version, tickerId];
    // send contract fields
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.TRADING_CLASS) {
        args.push(contract.conId);
    }
    ...
    if (lodash_1.default.isString(contract.secType) &&
        constants_1.default.BAG_SEC_TYPE.toUpperCase() === contract.secType.toUpperCase()) {
        if (!lodash_1.default.isArray(contract.comboLegs)) {
            args.push(0);
        }
    ...
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.SYNT_REALTIME_BARS) {
        args.push(keepUpToDate);
    }
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.LINKING) {
        args.push('');
    }
    this._send(args);
};
Outgoing.prototype.reqHistoricalTicks = function (tickerId, contract, startDateTime, endDateTime, numberOfTicks, whatToShow, useRTH, ignoreSize) {
    if (this._controller._serverVersion < 73) {
        return this._controller.emitError('It does not support historical tick data backfill.');
    }
    var args = [constants_1.default.OUTGOING.REQ_HISTORICAL_TICKS, tickerId];
    // send contract fields
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.TRADING_CLASS) {
        args.push(contract.conId);
    }
...
Outgoing.prototype.reqTickByTickData = function (tickerId, contract, tickType, numberOfTicks, ignoreSize) {
    ...
    var args = [constants_1.default.OUTGOING.REQ_TICK_BY_TICK_DATA, tickerId];
    // send contract fields
    args.push(contract.conId);
    args.push(contract.symbol);
    args.push(contract.secType);
...
Outgoing.prototype.cancelTickByTickData = function (tickerId) {
    ...
    var args = [constants_1.default.OUTGOING.CANCEL_TICK_BY_TICK_DATA, tickerId];
    this._send(args);
};
Outgoing.prototype.reqIds = function (numIds) {
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_IDS, version, numIds);
};
Outgoing.prototype.reqManagedAccts = function () {
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_MANAGED_ACCTS, version);
};
Outgoing.prototype.reqMarketDataType = function (marketDataType) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.REQ_MARKET_DATA_TYPE) {
        return this._controller.emitError('It does not support marketDataType requests.');
    }
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_MARKET_DATA_TYPE, version, marketDataType);
};
Outgoing.prototype.reqMktData = function (tickerId, contract, genericTickList, snapshot, regulatorySnapshot) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.SNAPSHOT_MKT_DATA && snapshot) {
        return this._controller.emitError('It does not support snapshot market data requests.');
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.UNDER_COMP) {
        return this._controller.emitError('It does not support delta-neutral orders.');
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.REQ_MKT_DATA_CONID) {
        return this._controller.emitError('It does not support conId parameter.');
    }
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.TRADING_CLASS) {
        if (!lodash_1.default.isEmpty(contract.tradingClass)) {
    ...
    var version = 11;
    var args = [constants_1.default.OUTGOING.REQ_MKT_DATA, version, tickerId];
    // send contract fields
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.REQ_MKT_DATA_CONID) {
        args.push(contract.conId);
    }
    ...
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.UNDER_COMP) {
        if (lodash_1.default.isPlainObject(contract.underComp)) {
            args.push(true);
            args.push(contract.underComp.conId);
            args.push(contract.underComp.delta);
            args.push(contract.underComp.price);
    ...
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.SNAPSHOT_MKT_DATA) {
        args.push(snapshot);
    }
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.REQ_SMART_COMPONENTS) {
...
Outgoing.prototype.reqMktDepth = function (tickerId, contract, numRows) {
    ...
    var version = 4;
    // send req mkt data msg
    var args = [constants_1.default.OUTGOING.REQ_MKT_DEPTH, version, tickerId];
    // send contract fields
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.TRADING_CLASS) {
        args.push(contract.conId);
    }
...
Outgoing.prototype.reqNewsBulletins = function (allMsgs) {
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_NEWS_BULLETINS, version, allMsgs);
};
Outgoing.prototype.reqOpenOrders = function () {
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_OPEN_ORDERS, version);
};
Outgoing.prototype.reqPositions = function () {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_POSITIONS, version);
};
Outgoing.prototype.reqPositionsMulti = function (reqId, account, modelCode) {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_POSITIONS_MULTI, version, reqId, account, modelCode);
};
Outgoing.prototype.reqRealTimeBars = function (tickerId, contract, barSize, whatToShow, useRTH) {
    ...
    var version = 2;
    // send req mkt data msg
    var args = [constants_1.default.OUTGOING.REQ_REAL_TIME_BARS, version, tickerId];
    // send contract fields
    if (this._controller._serverVersion >= constants_1.default.MIN_SERVER_VER.TRADING_CLASS) {
        args.push(contract.conId);
    }
...
Outgoing.prototype.reqScannerParameters = function () {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_SCANNER_PARAMETERS, version);
};
Outgoing.prototype.reqScannerSubscription = function (tickerId, subscription) {
    ...
    var version = 3;
    var args = [constants_1.default.OUTGOING.REQ_SCANNER_SUBSCRIPTION, version, tickerId];
...
Outgoing.prototype.requestFA = function (faDataType) {
    ...
    var version = 1;
    this._send(constants_1.default.OUTGOING.REQ_FA, version, faDataType);
};
Outgoing.prototype.setServerLogLevel = function (logLevel) {
    var version = 1;
    this._send(constants_1.default.OUTGOING.SET_SERVER_LOGLEVEL, version, logLevel);
};
Outgoing.prototype.queryDisplayGroups = function (reqId) {
    this._send(constants_1.default.OUTGOING.QUERY_DISPLAY_GROUPS, 1, reqId);
};
Outgoing.prototype.updateDisplayGroup = function (reqId, contractInfo) {
    this._send(constants_1.default.OUTGOING.UPDATE_DISPLAY_GROUP, 1, reqId, contractInfo);
};
Outgoing.prototype.subscribeToGroupEvents = function (reqId, groupId) {
    this._send(constants_1.default.OUTGOING.SUBSCRIBE_TO_GROUP_EVENTS, 1, reqId, groupId);
};
Outgoing.prototype.unsubscribeToGroupEvents = function (reqId) {
    this._send(constants_1.default.OUTGOING.UNSUBSCRIBE_FROM_GROUP_EVENTS, 1, reqId);
};
Outgoing.prototype.reqSecDefOptParams = function (reqId, underlyingSymbol, futFopExchange, underlyingSecType, underlyingConId) {
    if (this._controller._serverVersion < constants_1.default.MIN_SERVER_VER.SEC_DEF_OPT_PARAMS_REQ) {
        // We don't currently support the v100 extended handshake so our server version shows up as < 100.
        // However, this functionality still works so we skip the version check for now.
        //
        //return this._controller.emitError('It does not support reqSecDefOptParams.');
    }
    this._send(constants_1.default.OUTGOING.REQ_SEC_DEF_OPT_PARAMS, reqId, underlyingSymbol, futFopExchange, underlyingSecType, underlyingConId);
};
